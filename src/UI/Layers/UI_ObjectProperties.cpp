...
void UI_ObjectProperties::renderLayer()
{
    ...
    {
        transform = Scene::activeScene->getSelectedObject()->getComponent<TransformComponent>()->getTransform();
        ...
        {
            Scene::activeScene->getSelectedObject()->getComponent<TransformComponent>()->setPosition(transform.position);
        }
        if (ImGui::DragFloat3("Rotate", &transform.rotation.x, moveSpeed))
        {
            Scene::activeScene->getSelectedObject()->getComponent<TransformComponent>()->setRotation(transform.rotation);
        }
        if (ImGui::DragFloat3("Scale", &transform.scale.x, moveSpeed))
        {
            Scene::activeScene->getSelectedObject()->getComponent<TransformComponent>()->setScale(transform.scale);
        }
    }
    ImGui::End();
}
